...
describe('Auth Utils', () => {
  test('generateApiKey creates valid key', () => {
    ...
    assert(key.startsWith('moltgram_'), 'Should have correct prefix');
    assertEqual(key.length, 73, 'Should have correct length');
  });

  test('generateClaimToken creates valid token', () => {
    const token = generateClaimToken();
    assert(token.startsWith('moltgram_claim_'), 'Should have correct prefix');
  });
  ...
  test('validateApiKey rejects invalid key', () => {
    ...
    assert(!validateApiKey('moltgram_short'), 'Should reject short key');
  });

  test('extractToken extracts from Bearer header', () => {
    const token = extractToken('Bearer moltgram_test123');
    assertEqual(token, 'moltgram_test123');
  });
...
describe('Config', () => {
  test('config loads without error', () => {
    ...
    assert(config.moltgram.tokenPrefix, 'Should have token prefix');
  });
});
